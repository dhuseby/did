...
fn did_parse_example_2() {
    ...
    assert_eq!(doc.authentication[0].id, "did:example:123456789abcdefghi#keys-1");
    assert_eq!(doc.authentication[0].key_type, PublicKeyType::RsaVerificationKey2018);
    assert_eq!(doc.authentication[0].controller, "did:example:123456789abcdefghi");
    assert_eq!(doc.authentication[0].key_data_type, PublicKeyDataType::Pem);
    assert_eq!(doc.authentication[0].key_data, "-----BEGIN PUBLIC KEY...END PUBLIC KEY-----\r\n");
    assert_eq!(doc.authentication[0].reference, false);

    assert!(doc.service[0].context.is_missing());
    assert_eq!(doc.service[0].service_type, "VerifiableCredentialService");
    assert_eq!(doc.service[0].endpoint, "https://example.com/vc/");
    */
